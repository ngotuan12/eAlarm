...
function updateDeviceInfor(socket, device_id, infors)
{
	...
	strSQL += "WHERE a.device_pro_id = b.id " + "AND a.device_id = ? AND b.p_type = '1' ";
	...
	connDB
			.query(
					...
					function(err, rows)
					{
						...
						var strDescription = "";
						var strStatus = "1";
						strSQL = "UPDATE device_infor SET value = ? WHERE device_id = ? and device_pro_id = (SELECT id FROM device_properties WHERE code = ? AND p_type = '1' )";
						var properties = rows;
						var i;
...
{
	var strSQL = "SELECT * FROM device_properties WHERE p_type = '1'";
